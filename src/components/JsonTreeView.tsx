...
const Value = ({ value }: { value: any }) => {
  ...
  let color;
  switch (type) {
    case 'string': color = '#9ccc65'; break; // Green
    case 'number': color = '#4fc3f7'; break; // Blue
    case 'boolean': color = '#ff8a65'; break; // Orange
    case 'null': color = '#e0e0e0'; break; // Gray
    default: color = '#ba68c8'; break; // Purple for undefined
  }
  return <Typography component="span" sx={{ color, fontFamily: 'monospace' }}>{JSON.stringify(value)}</Typography>;
};

function JsonTreeView({ data, nodeKey = 'state', defaultOpen = false }: { data: any; nodeKey?: string; defaultOpen?: boolean }) {
  ...
  if (!isCollapsible) {
    return (
      <ListItem sx={{ pl: 2 }}>
        <ListItemText
          primary={<Typography component="span" sx={{ color: dataType === 'function' ? '#999' : '#ce9178', fontFamily: 'monospace' }}>"{nodeKey}": </Typography>}
  ...
  return (
    <List component="div" disablePadding sx={{ width: '100%' }}>
      <ListItem onClick={handleToggle} sx={{ p: '2px 8px' }}>
        ...
        <ListItemText
          primary={<Typography component="span" sx={{ color: '#ce9178', fontFamily: 'monospace' }}>"{nodeKey}": </Typography>}
          secondary={
