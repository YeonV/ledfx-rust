	...
	({
		...
		return (
			<>
					...
					<CardHeader
						...
						action={
							<>
								...
								<IconButton onClick={() => (isActive ? onStop(virtual.id) : onStart(virtual))}>
					...
					<CardContent sx={{ p: '8px', pb: '8px !important' }}>
						...
						<Stack direction={'row'} alignItems="center">
							...
							<Button
								disabled={!settings}
								variant="outlined"
								sx={{ mt: '12px', minWidth: 40, p: 0, height: 40, borderColor: '#444', color: 'text.primary' }}
								size="large"
								onClick={() => {
									setOpenEffectSettings(!openEffectSettings)
								}}
							>
								<TuneIcon />
							</Button>
						...
						{schema && settings && (
							<Collapse in={openEffectSettings} sx={{ mt: 0, p: 0, overflow: 'hidden', minHeight: 1 }}>
								<Box px={0}>
									<EffectSettings
										schema={schema}
										settings={settings}
				...
				<EditVirtualDialog open={isEditDialogOpen} onClose={() => setIsEditDialogOpen(false)} virtualToEdit={virtual} />
			</>
		)
	}
)
