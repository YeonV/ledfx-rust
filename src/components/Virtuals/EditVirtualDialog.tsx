...
export const EditVirtualDialog = ({ open, onClose, virtualToEdit }: EditVirtualDialogProps) => {
	...
	const [newSegment, setNewSegment] = useState<{ deviceId: string; start: number; end: number; length: number } | null>(
		null
	)
				...
				for (const cell of virtualToEdit.matrix_data[0]) {
					if (cell === null) {
						...
						if (currentSegment && currentSegment.deviceId === cell.device_id && currentSegment.end === cell.pixel - 1) {
							...
							if (currentSegment) decodedSegments.push(currentSegment)
							currentSegment = {
								id: `seg_${Date.now()}_${decodedSegments.length}`,
	...
	const handleSave = async () => {
		if (name && segments.length > 0) {
			...
			try {
				...
				console.error('Failed to save virtual:', e)
			}
		}
	}
		...
		<Dialog open={open} onClose={onClose} fullWidth maxWidth="sm">
			...
			<DialogContent>
					...
					<List dense>
						{segments.map((seg, index) => {
							const deviceName =
								seg.deviceId === 'GAP'
									? 'Gap'
									: devices.find((d) => d.ip_address === seg.deviceId)?.name || seg.deviceId
							const secondaryText =
								seg.deviceId === 'GAP' ? `${seg.length} pixels` : `Pixels ${seg.start} to ${seg.end}`
							return (
								<ListItem key={seg.id}>
				...
				{newSegment ? (
					<Box>
						<Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
							<FormControl fullWidth size="small">
								...
								<Select value={newSegment.deviceId} label="Type" onChange={(e) => handleDeviceChange(e.target.value)}>
									<MenuItem value="GAP">Gap</MenuItem>
									{devices.map((d) => (
										<MenuItem key={d.ip_address} value={d.ip_address}>
											{d.name}
										</MenuItem>
									))}
								</Select>
							...
							{newSegment.deviceId === 'GAP' ? (
								<TextField
									...
									onChange={(e) => setNewSegment({ ...newSegment, length: parseInt(e.target.value) || 0 })}
								/>
							) : (
								<>
									<TextField
									...
									<TextField
										...
										onChange={(e) => handleRangeChange('end', parseInt(e.target.value) || 0)}
										inputProps={{ min: 0, max: maxPixel }}
									/>
								</>
							)}
							<Button onClick={handleAddSegment} variant="contained" size="large">
								Add
							</Button>
						</Box>
						{overlapError && (
					...
					<Button
						startIcon={<AddIcon />}
						onClick={() => setNewSegment({ deviceId: devices[0]?.ip_address || 'GAP', start: 0, end: 0, length: 10 })}
					>
						Add Segment
					</Button>
				)}
			...
			<DialogActions>
				...
				<Button onClick={handleSave} variant="contained" disabled={segments.length === 0}>
					Save
				</Button>
			</DialogActions>
		</Dialog>
