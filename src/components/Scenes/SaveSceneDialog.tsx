...
export const SaveSceneDialog = ({ open, onClose }: SaveSceneDialogProps) => {
	...
	const handlePresetSelectionForVirtual = (virtualId: string, presetName: string) => {
		...
		if (presetName === 'CUSTOM') {
			...
			setVirtualSceneEffects((prev) => ({
				...prev,
				[virtualId]: { type: 'preset', data: { effect_id: effectId, preset_name: presetName } }
			}))
		}
	}
		...
		<Dialog open={open} onClose={onClose} fullWidth maxWidth="sm">
			...
			<DialogContent>
				...
				<Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
					Select which effects to include and whether to save them as a preset or as custom settings.
				</Typography>
				<List dense sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 1 }}>
					{Object.keys(activeEffects)
						.filter((id) => activeEffects[id])
						.map((virtualId, index) => {
								...
								<>
									...
									<ListItem key={virtualId} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
										<Box sx={{ display: 'flex', alignItems: 'center', width: '100%' }}>
											<Checkbox
												edge="start"
												checked={!!includedVirtuals[virtualId]}
												onChange={(e) => setIncludedVirtuals((prev) => ({ ...prev, [virtualId]: e.target.checked }))}
											/>
										...
										<Box sx={{ pl: 6, width: '100%', pb: 1 }}>
											<FormControl fullWidth size="small" margin="dense">
												...
												<Select
													value={getPresetValueForVirtual(virtualId)}
													label="Saved As"
													onChange={(e) => handlePresetSelectionForVirtual(virtualId, e.target.value)}
												>
