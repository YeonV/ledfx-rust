...
import { commands, DspSettings } from '../../bindings';
...
const CurveVisualizer = ({ settings }: CurveVisualizerProps) => {
  ...
  useEffect(() => {
    if (settings.filterbank_type && typeof settings.filterbank_type === 'object' && 'BladePlus' in settings.filterbank_type) {
  ...
  return (
    <Box sx={{ mb: 2, p: 1, border: '1px solid', borderColor: 'divider', borderRadius: 1, backgroundColor: 'rgba(0,0,0,0.2)' }}>
      <svg width="100%" height={SVG_HEIGHT} viewBox={`0 0 ${SVG_WIDTH} ${SVG_HEIGHT}`}>
        {/* Background Grid */}
        <line x1={0} y1={SVG_HEIGHT/2} x2={SVG_WIDTH} y2={SVG_HEIGHT/2} stroke="#444" strokeWidth="0.5" strokeDasharray="4" />
        <line x1={SVG_WIDTH/2} y1={0} x2={SVG_WIDTH/2} y2={SVG_HEIGHT} stroke="#444" strokeWidth="0.5" strokeDasharray="4" />
