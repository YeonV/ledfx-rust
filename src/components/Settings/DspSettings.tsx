...
const DspSettings = () => {
	...
	const isDirty = useMemo(() => {
		...
		return (
			dspSettings.fft_size !== dirtyDspSettings.fft_size ||
	...
	const handleBladePlusChange = (key: keyof BladePlusParams, value: number) => {
		if (dirtyDspSettings) {
			...
			if (
				newDirtySettings.filterbank_type &&
				typeof newDirtySettings.filterbank_type === 'object' &&
				'BladePlus' in newDirtySettings.filterbank_type
			) {
	...
	const handleApplyCriticalChanges = () => {
		...
		commands
			.updateDspSettings(dirtyDspSettings)
			.then(() => {
	...
	const handleFilterbankChange = (newValue: string) => {
		...
		if (newValue === 'BladePlus') {
			newFilterbankType = {
				BladePlus:
					(typeof dirtyDspSettings.filterbank_type === 'object' &&
						'BladePlus' in dirtyDspSettings.filterbank_type &&
						dirtyDspSettings.filterbank_type.BladePlus) ||
					defaultBladePlusParams
			}
		} else {
	...
	const currentBladePlusParams =
		typeof dirtyDspSettings.filterbank_type === 'object' && 'BladePlus' in dirtyDspSettings.filterbank_type
			? dirtyDspSettings.filterbank_type.BladePlus
	...
	return (
		<>
			<Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', p: 2, pt: 0 }}>
				...
				<Button startIcon={<RestartAltIcon />} onClick={handleResetToDefaults}>
					Reset All
				</Button>
			</Box>

			<Typography variant="subtitle1" sx={{ p: 2, pt: 0, pb: 1, fontWeight: 'bold' }}>
				Analysis
			</Typography>
			<SettingsRow icon={<TuneIcon />} title="FFT Size">
				<FormControl fullWidth size="small">
					<Select
						value={dirtyDspSettings.fft_size}
						onChange={(e) => handleSettingChange('fft_size', Number(e.target.value))}
					>
				...
				<IconButton size="small" onClick={() => handleResetSingle('fft_size')}>
					<RestartAltIcon />
				</IconButton>
			</SettingsRow>
			<SettingsRow icon={<EqualizerIcon />} title="Frequency Bands">
				<FormControl fullWidth size="small">
					<Select
						value={dirtyDspSettings.num_bands}
						onChange={(e) => handleSettingChange('num_bands', Number(e.target.value))}
					>
				...
				<IconButton size="small" onClick={() => handleResetSingle('num_bands')}>
					<RestartAltIcon />
				</IconButton>
			</SettingsRow>
			<SettingsRow icon={<HubIcon />} title="Sample Rate">
				<Tooltip title="Overrides the native device sample rate. 'Device Native' is recommended.">
					<FormControl fullWidth size="small">
						<Select
							value={dirtyDspSettings.sample_rate || 0}
							onChange={(e) =>
								handleSettingChange('sample_rate', Number(e.target.value) === 0 ? null : Number(e.target.value))
							}
						>
				...
				<IconButton size="small" onClick={() => handleResetSingle('sample_rate')}>
					<RestartAltIcon />
				</IconButton>
			</SettingsRow>
			<SettingsRow icon={<GraphicEqIcon />} title="Filterbank Type">
				<FormControl fullWidth size="small">
					<Select
						value={getFilterbankTypeValue(dirtyDspSettings.filterbank_type)}
						onChange={(e) => handleFilterbankChange(e.target.value)}
					>
				...
				<IconButton size="small" onClick={() => handleResetSingle('filterbank_type')}>
					<RestartAltIcon />
				</IconButton>
			</SettingsRow>
			...
			<Collapse in={getFilterbankTypeValue(dirtyDspSettings.filterbank_type) === 'BladePlus'}>
				<Box sx={{ p: 2, border: '1px solid', borderColor: 'divider', m: 2, borderRadius: 1 }}>
					<Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
						...
						<Button size="small" startIcon={<RestartAltIcon />} onClick={handleResetBladePlusCurve}>
							Reset Curve
						</Button>
					...
					<SettingsRow title={`Curve Base: ${currentBladePlusParams.log_base.toFixed(1)}`}>
						<Slider
							value={currentBladePlusParams.log_base}
							onChange={(_e, v) => handleBladePlusChange('log_base', v as number)}
							min={2}
							max={20}
							step={0.1}
						/>
					...
					<SettingsRow title={`Low-End Focus: ${currentBladePlusParams.multiplier.toFixed(0)}`}>
						<Slider
							value={currentBladePlusParams.multiplier}
							onChange={(_e, v) => handleBladePlusChange('multiplier', v as number)}
							min={1000}
							max={5000}
							step={50}
						/>
					...
					<SettingsRow title={`Rolloff Point: ${currentBladePlusParams.divisor.toFixed(0)}`}>
						<Slider
							value={currentBladePlusParams.divisor}
							onChange={(_e, v) => handleBladePlusChange('divisor', v as number)}
							min={50}
							max={1000}
							step={10}
						/>
			...
			<SettingsRow
				icon={<VolumeUpIcon />}
				title={`Freq Range: ${dirtyDspSettings.min_freq}Hz - ${dirtyDspSettings.max_freq}Hz`}
			>
				<Slider
					value={[dirtyDspSettings.min_freq, dirtyDspSettings.max_freq]}
					onChange={(_e, val) => {
				...
				<IconButton
					size="small"
					onClick={() => {
						handleResetSingle('min_freq')
						handleResetSingle('max_freq')
					}}
				>
					<RestartAltIcon />
				</IconButton>
			...
			<Collapse in={isDirty}>
				...
				>
					<Alert severity="warning" sx={{ mb: 2 }}>
						You have unapplied critical settings.
					</Alert>
