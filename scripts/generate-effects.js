...
function generateEngineFileContent(effects) {
    ...
    const getEffectIdArms = effects.map(name =>
        `        EffectConfig::${toCamelCase(name)}(_) => "${name}".to_string(),`
    ).join('\n');
    
    const builtInPresetsArms = effects.map(name =>
        `        "${name}" => effects::${name}::get_built_in_presets(),`
    ).join('\n');
