...
fn main() -> io::Result<()> {
    println!("cargo:rerun-if-changed=src/effects");

    // 1. Discover effect modules
    ...
    // 3. Generate `engine/generated.rs`
    {
        ...
        writeln!(file, "#[derive(Deserialize, Serialize, Type, Clone)]")?;
        ...
        for effect in &effects {
            let variant_name = effect.to_upper_camel_case();
            writeln!(file, "        EffectInfo {{ id: \"{}\".to_string(), name: effects::{}::NAME.to_string(), variant: \"{}\".to_string() }},", effect, effect, variant_name)?;
        }
        writeln!(file, "    ])\n}}\n")?;
