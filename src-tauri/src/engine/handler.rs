...
use super::generated::{create_effect, config_to_value, get_built_in_presets_for_effect};
...
pub fn handle_command(
    ...
    match command {
        ...
        EngineCommand::UpdateDspSettings { settings } => {
            ...
            audio_command_tx.send(AudioCommand::UpdateSettings(settings.clone())).unwrap();
        ...
        EngineCommand::ReloadState => {
            ...
            *virtuals = engine_state.virtuals.clone().into_iter().map(|(id, config)| {
        ...
        EngineCommand::AddDevice { config } => {
            ...
            let matrix_data = vec![(0..config.led_count).map(|i| { Some(MatrixCell { device_id: device_ip.clone(), pixel: i }) }).collect()];
            let device_virtual = Virtual { id: virtual_id.clone(), name: config.name.clone(), matrix_data, is_device: Some(device_ip.clone()) };
            let pixel_count = device_virtual.matrix_data.iter().flat_map(|row| row.iter()).filter(|cell| cell.is_some()).count();
            virtuals.insert(virtual_id, ActiveVirtual { effect: None, config: device_virtual, pixel_count, r_channel: vec![0.0; pixel_count], g_channel: vec![0.0; pixel_count], b_channel: vec![0.0; pixel_count] });
            should_save_state = true;
            emit_devices_update(devices, app_handle);
            emit_virtuals_update(virtuals, app_handle);
        }
        ...
        EngineCommand::AddVirtual { config } => {
            let pixel_count = config.matrix_data.iter().flat_map(|row| row.iter()).filter(|cell| cell.is_some()).count();
            virtuals.insert(config.id.clone(), ActiveVirtual { effect: None, config, pixel_count, r_channel: vec![0.0; pixel_count], g_channel: vec![0.0; pixel_count], b_channel: vec![0.0; pixel_count] });
            should_save_state = true;
            emit_virtuals_update(virtuals, app_handle);
        }
                    ...
                    if let Some(active_virtual) = virtuals.get_mut(virtual_id) {
                        let effect_config = match scene_effect {
                            ...
                            SceneEffect::Preset(scene_preset) => {
                                engine_state.effect_presets.get(&scene_preset.effect_id)
