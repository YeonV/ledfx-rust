...
pub fn get_playback_state(state_tx: State<EngineStateTx>) -> Result<PlaybackState, String> {
    ...
    state_tx
        .0
        .send(super::state::EngineRequest::GetPlaybackState(responder_tx))
        .map_err(|e| e.to_string())?;
...
pub fn get_virtuals(state_tx: State<EngineStateTx>) -> Result<Vec<Virtual>, String> {
    ...
    state_tx
        .0
        .send(super::state::EngineRequest::GetVirtuals(responder_tx))
        .map_err(|e| e.to_string())?;
    responder_rx.recv().map_err(|e| e.to_string())
}
#[tauri::command]
...
pub fn get_devices(state_tx: State<EngineStateTx>) -> Result<Vec<Device>, String> {
    ...
    state_tx
        .0
        .send(super::state::EngineRequest::GetDevices(responder_tx))
        .map_err(|e| e.to_string())?;
    responder_rx.recv().map_err(|e| e.to_string())
}
#[tauri::command]
...
pub fn load_presets(
    ...
    engine_state_tx
        .0
        .send(super::state::EngineRequest::GetPresets(
            effect_id,
            responder_tx,
        ))
        .map_err(|e| e.to_string())?;
